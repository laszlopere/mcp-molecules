...
dir) and the online fallback explicitly off (it is on by default at runtime), so
tests never read the developer's real ``~/.local/share`` cache nor touch the
network unless they opt in by setting ``MCP_MOLECULES_ONLINE`` back on.
"""
